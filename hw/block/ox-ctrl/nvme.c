...
static int nvme_init_ctrl (NvmeCtrl *n)
{
    ...
    uint8_t *pci_conf = core.qemu->parent_obj.config;
    ...
    id->oncs = cpu_to_le16(NVME_ONCS_FEATURES);
    ...
    id->cntlid = cpu_to_le16(0xaaac);

    /* Fields not defined yet */

    id->ver = cpu_to_le32(0);
    id->rtd3r = 0;
    id->rtd3e = 0;
    id->ctratt = 0;
    id->avscc = 0;
    id->apsta = 0;
    id->wctemp = cpu_to_le16(0);
    id->cctemp = cpu_to_le16(0);
    id->mtfa = 0;
    id->hmpre = 0;
    id->hmmin = 0;
    id->tnvmcap[0] = 0;
...
static int nvme_init_namespaces (NvmeCtrl *n)
{
	...
	for (j = 0; j < n->nlbaf; j++) {
            ...
            id_ns->lbaf[j].ms = (oob_sz > sec_sz) ?
                                    cpu_to_le16(sec_sz) : cpu_to_le16(oob_sz);
	}
	...
	ns->start_block = 0;
        ...
        id_ns->rescap = 0;
        id_ns->fpi = 0;
        id_ns->nawun = cpu_to_le16(0);
        id_ns->nawupf = cpu_to_le16(0);
        id_ns->nacwu = cpu_to_le16(0);
        id_ns->nabsn = cpu_to_le16(0);
        id_ns->nabo = cpu_to_le16(0);
        id_ns->nabspf = cpu_to_le16(0);
        id_ns->nvmcap[0] = cpu_to_le16(0);
    }
...
void nvme_addr_write (NvmeCtrl *n, uint64_t addr, void *buf, int size)
{
    if (n->cmbsz && addr >= core.qemu->ctrl_mem.addr &&
                addr < (core.qemu->ctrl_mem.addr +
                int128_get64(core.qemu->ctrl_mem.size))) {
        memcpy((void *)&n->cmbuf[addr - core.qemu->ctrl_mem.addr], buf, size);
        return;
    } else {
        pci_dma_write(&core.qemu->parent_obj, addr, buf, size);
    }
}
...
void nvme_process_db (NvmeCtrl *n, uint64_t addr, uint64_t val)
{
    ...
    if (((addr - 0x1000) >> (2 + n->db_stride)) & 1) {
        ...
        if (start_sqs) {
            NvmeSQ *sq;
            TAILQ_FOREACH(sq, &cq->sq_list, entry) {
